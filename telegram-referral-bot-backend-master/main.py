...
new_hostname = "mongodb+srv://spark:spark123@cluster0.iszbnmf.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"

# Database connection
client = MongoClient(new_hostname)
db = client["airdrop_db"]
users_collection = db["users"]
tasks_collection = db["tasks"]
user_activity_collection = db["user_activity"]

TELEGRAM_BOT_TOKEN = "7106982721:AAFqsx85J6a0dlNy28Ld8JYPmlCAlAN2uu8"
...
class Verifier:
    ...
    telegram_chat_id = "-1001916965087"
...
@app.post("/points/{telegram_uid}")
def update_points(telegram_uid: str, points: int = Body(...)):
    user = users_collection.find_one({"telegram_uid": telegram_uid})
    if user:
        user["_id"] = str(user["_id"])
        user["points"] = points
        ...
        return user["telegram_uid"], user["points"]
    else:
        raise HTTPException(status_code=404, detail="User not found")
